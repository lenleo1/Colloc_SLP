clc; clear;
problem.type = 1; % min tf
% dynamics
problem.sys = @dubins_dynamics_control;

% collocation grid size
problem.nGrid = 21;
problem.nState = 4; % x v theta Q
...
problem.bounds.x_low = [-4;  -4;  -pi; 0];
problem.bounds.x_upp = [ 4;   4;   pi; 20];
problem.bounds.xIBC_low = [-3; -2.5; pi/2; 0];
problem.bounds.xIBC_upp = [-3; -2.5; pi/2; 0];
problem.bounds.xFBC_low = [3; 2.5; 0; 3];
problem.bounds.xFBC_upp = [3; 2.5; 0; 20];
