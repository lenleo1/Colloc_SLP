...
% dynamics
problem.sys = @double_integrator_dynamics;

% collocation grid size
problem.nGrid = 21;
problem.nState = 2; % x v
problem.nControl = 1; % u
problem.nOutput = 0; % y

% Problem bounds
problem.bounds.Tf_low = 10;
problem.bounds.Tf_upp = 35;
problem.bounds.x_low = [-10;  -200];
problem.bounds.x_upp = [ 300;  200];
problem.bounds.xIBC_low = [0; 0];
problem.bounds.xIBC_upp = [0; 0];
problem.bounds.xFBC_low = [300; 0];
problem.bounds.xFBC_upp = [300; 0];
problem.bounds.u_low = -2;
problem.bounds.u_upp =  1;
...
problem.tf = 30;
problem.controls = repmat(0.5, 1, problem.nGrid);
...
problem.history.rho_sum = nan(iter_num,1);
